...
import Pop from "../Pop";
import "../../css/drop.css";
const DrobCenters = (props) => {
  const [open, setOpen] = useState(false);
  const [label, setLabel] = useState(props.label);
  const [valueCenter, setValueCenter] = useState(props.value);
  useEffect(() => {
    props.updateValue(valueCenter);
  }, [valueCenter, props]);
  return (
    <>
  ...
  return (
    <motion.li
      ...
      onClick={() => {
        setOpen(false);
        setLabel(text);
        setValue(text); // Set the label as text.name
      }}
